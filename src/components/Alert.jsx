...
const Alert = ({ isShow, color = 'green', context = 'Post has been published!' }) => {
  const dispatch = useDispatch();
  const { alertValue } = useSelector(selectPost);
  const onCloseAlert = () => {
    dispatch(postAction.setAlertValue({ ...alertValue, isShow: false }));
  };

  return (
    <div
      className={`${isShow ? 'flex' : 'hidden'} items-center max-w-lg fixed bottom-10 right-[50%] translate-x-1/2 z-50 justify-between p-4 text-${color}-700 border rounded border-${color}-900/10 bg-${color}-50`}
      role='alert'>
